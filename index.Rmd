...
This table shows the site summaries for all combinations of meeting or not meeting the biostimulatory targets.  The totals indicate the number of sites in each category and the proportion of those sites for which all selected biointegrity goals are met.  Totals are separated by calibration and validation sites.  Rows can be sorted using the selection arrows next to each column name (e.g., sort by a biostimulatory variable to see other variables that were not met).
```{r}
renderDataTable({
