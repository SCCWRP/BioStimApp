...
column(12, 
       ...
       column(6, 
              selectInput("selpr", label = 'Select probability of meeting goal:', choices = unipr, selected = 'p80'
                                    )
       )
...
# get all combinations from 1 to n for selected bs variables
cmbsrct <- reactive({
...
bsbirct <- reactive({
  ...
  alltlly <- bsbidat %>% 
    ...
    mutate(
      bsfail = ifelse(bsval > Est, 'unmet', 'met'),
      bipass = bival >= glval
    ) %>% 
  ...
  out <- sumtlly %>% 
    ...
    rename(
      ...
      `Proportion passing (cal)` = Cal_bipassprp,
      `Total sites (val)` = Val_bipassall,
      `Proportion passing (val)` = Val_bipassprp
    )
...
renderTable({
  ...
  trgtrct <- trgtrct %>% 
    rename(
      `Biostimulatory variable` = BSPretty,
...
This table shows the site summaries for all combinations of meeting or not meeting the biostimulatory targets.  The totals indicate the number of sites in each category and the proportion of those sites for which all biointegrity goals are met.  Totals are separated by calibration and validation sites. 
```{r}
renderDataTable({
...
}, rownames = F, options = list(dom = 't', pageLength = 32))

# # calibration plot
# renderPlot({
# 
#   # input
#   bsbirctcal <- bsbirctcal()
# 
#   # grid plot
#   toplo1 <- bsbirctcal %>%
#     select(-DevSet, -bipasstot, -bipassprp) %>%
#     gather('bsvar', 'bsval', -cnt, -ind)
# 
#   p1 <- ggplot(toplo1, aes(x = bsvar, y = ind, fill = bsval)) +
#     geom_tile()
#   return(bsbirctcal)
#   })
```
