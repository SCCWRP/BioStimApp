...
output$sumplo <- renderPlot({
  ...
  p1 <- ggplot(toplo1, aes(x = name, y = '1')) + 
    ...
    coord_flip() +
    scale_x_discrete('Total biostim thresholds met', expand = c(0, 0), labels = rev(toplo1$cnt)) +
  ...
  toplo2 <- bsbirct %>% 
    ...
    mutate(var = factor(var, levels = c('bipassprprec', 'bipassprp')))
    
  # bar plot
  p2 <- ggplot(toplo2, aes(x = name, y = val, fill = var)) + 
    geom_bar(stat = 'identity', width = 1, colour = '#f0f0f090') + 
    facet_wrap(~calval) +
  ...
  p1 + p2 + plot_layout(ncol = 2, widths = c(0.35, 1))
